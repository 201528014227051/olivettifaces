...
Run on GPU:THEANO_FLAGS=mode=FAST_RUN,device=gpu,floatX=float32 python test.py
...
def load_data(dataset_path):
	...
	for row in range(20):
		for colum in range(20):
			faces[row*20+colum] = numpy.ndarray.flatten(img_ndarray [row*57:(row+1)*57,colum*47:(colum+1)*47])
